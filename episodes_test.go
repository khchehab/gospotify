...
func TestGetEpisode_Success(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"id":"ep1","name":"Episode 1","type":"episode","duration_ms":1800000}`))
	}))
	defer srv.Close()
...
func TestGetEpisode_APIError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"error":{"status":404,"message":"Episode not found"}}`))
	}))
	defer srv.Close()
...
func TestGetUserSavedEpisodes_WithOptions(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"href":"","total":0,"limit":5,"offset":2,"items":[]}`))
	}))
	defer srv.Close()
